...
class Edge:
    ...
    def _detect_next_stop_or_sell_point(self, buy_column, sell_column, date_column, ohlc_columns, stoploss, pair: str):
        ...
        while True:
            try:
                open_trade_index = list(buy_column).index(1)
            except ValueError:
                break
            ...
            try:
                stop_index = next(i for i, v in enumerate(ohlc_columns[open_trade_index:, 2]) if v < stop_price)
            except StopIteration:
                stop_index = float("inf")

            try:
                sell_index = list(sell_column[open_trade_index:]).index(1)
            except ValueError:
