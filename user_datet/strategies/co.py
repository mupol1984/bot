...
def SSLChannels_ATR(dataframe:DataFrame, length=7):
    df = dataframe.copy()
...
class co(IStrategy):
    ...
    sell_params = {
        ...
        "cstop_bail_how": "roc",
        "cstop_bail_roc": -2.18,
        "cstop_bail_time": 1100,
        "cstop_bail_time_trend": False,
        "cstop_loss_threshold": -0.232,
    }

    # Protection hyperspace params:
    protection_params = {
        "cooldown_lookback": 5,  # value loaded from strategy
        "max_allowed_drawdown": 0.2,  # value loaded from strategy
        "max_drawdown_lookback": 12,  # value loaded from strategy
        "max_drawdown_stop_duration": 12,  # value loaded from strategy
        "max_drawdown_trade_limit": 5,  # value loaded from strategy
        "stoploss_guard_lookback": 12,  # value loaded from strategy
        "stoploss_guard_stop_duration": 12,  # value loaded from strategy
        "stoploss_guard_trade_limit": 3,  # value loaded from strategy
    }
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            ((dataframe['consensus_buy'] < self.buy_score_short.value)),'enter_short'] = 1
